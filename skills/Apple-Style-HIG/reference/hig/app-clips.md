...
![A sketch of an app icon surrounded by a dashed line, suggesting an App Clip. The image is overlaid with rectangular and circular grid lines and is tinted blue to subtly reflect the blue in the original six-color Apple logo.]
...
![A screenshot of an iPhone Lock Screen. The bottom half of the screen shows the App Clip card for a food truck’s App Clip as it appears when the person invokes the App Clip.]

![A screenshot of a food truck’s App Clip on iPhone as it appears when a person confirms the App Clip’s launch on the App Clip card. The App Clip displays a list with various donuts the person can order.]
...
![A horizontal row of two App Clip cards. The left App Clip card is for a game and uses Play as the verb for the action button. The right App Clip card is for an app and uses Open as the verb for the action button.]
...
![An App Clip Code that uses a badge design with the App Clip logo.]

![An App Clip Code that uses a design without the App Clip logo.]
...
![A scan-only App Clip Code with callouts for the center icon, visual code, and the App Clip logo.]
...
![An illustration that shows a circle that represents a cylindrical surface. Lines divide the circle into six segments of equal size. One segment represents an App Clip Code and shows how the code doesn’t cover more than one-sixth, or 60 degrees, of the surface’s circumference.]
...
![A correctly placed App Clip Code in the upright position.]

![A checkmark in a circle to indicate correct usage.]

![An incorrectly placed App Clip Code that's rotated 90 degrees to the left.]

![An X in a circle to indicate an invalid App Clip Code.]

![An incorrectly placed App Clip Code that's rotated 135 degrees to the right.]
...
![An App Clip Code that uses the badge design and has a minimum diameter of 3/4 inch (1.9 cm).]

![An App Clip Code that uses the design without the App Clip logo and has a minimum diameter of 3/4 inch (1.9 cm).]
...
![An illustration of an App Clip Code next to a QR code. Red guides denote that both are the same size.]
...
![An illustration that shows an App Clip Code with the badge design to the left of an App Clip Code without the App Clip logo. A red guide surrounds each App Clip Code, illustrating the clear space requirements.]
...
![Four App Clip badges, each using different colors. The two on the left use the badge design, and the two on the right use the design without the App Clip logo.]
...
![An illustration of an invalid App Clip Code with a changed aspect ratio.]

![An X in a circle to indicate an invalid App Clip Code.]

![An illustration of an invalid App Clip Code with a color gradient instead of a solid background color.]

![An X in a circle to indicate an invalid App Clip Code.]

![An illustration of an invalid App Clip Code with a drop shadow effect.]
...
![An illustration of an App Clip Code that uses the badge design and has callouts for the background, foreground, and generated colors.]
