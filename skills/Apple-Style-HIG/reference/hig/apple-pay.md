...
![A screenshot of an Apple Pay payment sheet that displays details about a food truck purchase, including a payment method and the total amount.]
...
![An illustration of an Apple Pay order confirmation screen on iPhone. The screen contains a button for creating an account, a button for signing up with Apple, and existing account login fields.]
...
![A screenshot of a webpage payment sheet that includes an additional charge for gift wrap and a credit applied for a coupon.]
...
![A screenshot of an Apple Pay payment sheet on iPhone, which shows a website icon above the payment details.]
...
![A screenshot of a webpage Apple Pay payment sheet that shows an error with the shipping address.]

![A screenshot of a webpage Apple Pay payment sheet that shows an error with the shipping address. An overlay appears over the payment sheet and denotes the postal code doesn't match the country for the selected address. Options exist to select a different shipping address or edit the shipping address.]
...
![A screenshot of a webpage Apple Pay payment sheet for a fixed subscription, which includes a monthly amount.]
...
![A screenshot of an in-app Apple Pay payment sheet for a fixed subscription that doesn’t require payment until after a three-month trial. The total shows a zero dollar amount.]

**Web**

![A screenshot of a webpage Apple Pay payment sheet for a fixed subscription that doesn’t require payment until after a three-month trial. The total shows a zero dollar amount.]
...
![Apple Pay button displaying an image of someone's default payment card.]
...
| ![Donate with Apple Pay button] | An app or website for an [Approved nonprofits](https://developer.apple.com/support/apple-pay-nonprofits/) that lets people make donations. |
...
![An illustration showing a Check Out with Apple Pay button above a custom Add to Cart button. Both buttons have the maximum corner radius, which results in a capsule-shaped appearance.]
...
| ![Incorrect usage] | Purchase with APPLE PAY (when not conforming to an interface style that uses only capital letters) |
